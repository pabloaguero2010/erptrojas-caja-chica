...
if($val)
	{
	header("location: operaciones.php?op=ecl2".$campos.$val);
	}
else
	{
	...
	$sql = "UPDATE cliente SET nombre_cliente='$nombre', giro_cliente='$giro', direccion_cliente='$direccion', telefono_cliente='$fono1', telefono2_cliente='$fono2', telefono3_cliente='$fono3', mail_cliente='$mail', web_cliente='$web' WHERE rut_cliente = '$rut'";
	$stat = pg_exec($dbh,$sql);
	/*if($_POST[proyecto])
		{
	...
	if($stat)
		{
		$val="&ok=1";
		header("location: operaciones.php?op=ecl".$val);
		}
	else
		{
		$val="&bd=1";
	
		header("location: operaciones.php?op=ecl2&cod=".$rut.$campos.$val);
		}
	}
