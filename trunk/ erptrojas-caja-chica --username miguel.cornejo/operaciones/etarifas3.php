...
$escolta=-1;
	...
	$sql = "UPDATE tarifa SET normal = '$normal', urgente = '$urgente', retorno = '$retorno', sobredimen = '$sobredimen', sobrepeso = '$sobrepeso', desmov = '$desmov', escolta = '$escolta' WHERE cod_tarifa = '$cod'";
	...
	if($stat)
		{
		$val="&ok=1";
		header("location: operaciones.php?op=etarifas".$val);
		}
	else
		{
		$val="&bd=1";
		//$campos="&cliente".$_POST["cliente"]."&nombre=".$_POST["nombre"];
		//echo $sql2;
		header("location: operaciones.php?op=etarifas".$campos.$val);
		}
	}
